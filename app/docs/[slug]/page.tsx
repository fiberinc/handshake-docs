...
import { getProviderWithDocs, getProviders } from '~/lib/providers';
import { PostRender } from './PostRender';
import { makeSerializeOptions } from './serialize-options';
...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
	const post = await getProviderWithDocs(params.slug);
...
export default async function Page({ params }: Props) {
	const post = await getProviderWithDocs(params.slug, makeSerializeOptions());
	if (!post) {
		return notFound();
	}
